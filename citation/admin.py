...
class PublicationCuratorForm(ActionForm):
    assigned_curator_id = forms.ModelChoiceField(
        queryset=User.objects.filter(is_active=True), label="User Name"
    )
...
class ManyRelatedFilterMixin:
    ...
    def get_queryset(self, request=None):
        ...
        return self.model.objects.annotate(has_pubs=Exists(publications)).filter(
            has_pubs=True
        )
