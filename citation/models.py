    ...
    for column_name, new_raw_value in changes.items():
        ...
        if new_value != old_value:
            ...
            if field.many_to_one and field.related_model != User:
                ...
                labels[field.name] = {"old": old_label, "new": new_label}
...
class InvitationEmailTemplate(models.Model):
    ...
    added_by = models.ForeignKey(
        User, related_name="citation_added_by", on_delete=models.PROTECT
    )
...
class Author(AbstractLogModel):
    ...
    date_added = models.DateTimeField(
        auto_now_add=True, help_text=_("Date this model was imported into this system")
    )
    date_modified = models.DateTimeField(
        auto_now=True, help_text=_("Date this model was last modified on this system")
    )
    ...
    def __repr__(self):
        return "Author(id={id}, orcid={orcid}, email={email}, given_name={given_name}, family_name={family_name})".format(
            id=self.id,
            orcid=repr(self.orcid),
            given_name=repr(self.given_name),
...
class AuthorAlias(AbstractLogModel):
    ...
    author = models.ForeignKey(
        Author, on_delete=models.PROTECT, related_name="author_aliases"
    )
    ...
    def __repr__(self):
        return "AuthorAlias(id={id}, family_name={family_name}, given_name={given_name}, author_id={author_id})".format(
            id=self.id,
            family_name=repr(self.family_name),
            given_name=repr(self.given_name),
...
class AuthorCorrespondenceLogQuerySet(models.QuerySet):
    def create_from_publications(
        self, publication_qs, custom_content="", curator=None, create=True
    ):
        author_correspondence = []
        for publication in publication_qs:
            author_correspondence.append(
                AuthorCorrespondenceLog.from_publication(
                    publication=publication, content=custom_content, curator=curator
                )
            )
        if create:
...
class CodeArchiveStatus(Enum):
    ...
    ARCHIVED = (
        3,
        "Code is archived in a trusted digital repository",
...
class AuthorCorrespondenceLog(models.Model):
    CODE_ARCHIVE_STATUS = Choices(*[(s.name, s.message) for s in CodeArchiveStatus])
    ...
    email_delivery_status = models.CharField(
        max_length=50, choices=DELIVERY_STATUS, default=DELIVERY_STATUS.not_sent
    )
    author_submitted_url = models.URLField(help_text=_("Code archive URL"), blank=True)
    author_feedback = models.TextField(
        help_text=_("Correspondence / feedback / corrections to this metadata record"),
        blank=True,
    )
    ...
    def from_publication(cls, publication, content=None, curator=None):
        return AuthorCorrespondenceLog(
    ...
    def create_email_text(self, request=None):
        ...
        context = dict(
            correspondence_url=correspondence_url, author_correspondence_log=self
        )
...
class ModelDocumentation(AbstractLogModel):
    CATEGORIES = [
        ...
        },
        {
            ...
            "modelDocumentationList": [
                {"category": "Visual Relationships", "name": "UML"},
...
class Note(AbstractLogModel):
    ...
    added_by = models.ForeignKey(
        User, related_name="citation_added_note_set", on_delete=models.PROTECT
    )
    deleted_on = models.DateTimeField(null=True, blank=True)
    deleted_by = models.ForeignKey(
        User,
        related_name="citation_deleted_note_set",
        null=True,
        blank=True,
        on_delete=models.SET_NULL,
    )
    publication = models.ForeignKey(
        "Publication", null=True, blank=True, on_delete=models.SET_NULL
    )
...
class Container(AbstractLogModel):
    ...
    def __repr__(self):
        return "Container(id={id}, name={name}, issn={issn}, type={type})".format(
            id=self.id, name=repr(self.name), issn=repr(self.issn), type=repr(self.type)
        )

    def get_message(self):
        return "name: {} issn: {}".format(
            repr(self.name), repr(self.issn) if self.issn else "''"
        )

    def duplicates(self):
        return Container.objects.filter(
            (Q(issn=self.issn) & Q(issn__isnull=False))
...
class ContainerAlias(AbstractLogModel):
    ...
    container = models.ForeignKey(
        Container, on_delete=models.PROTECT, related_name="container_aliases"
    )

    def __repr__(self):
        return "ContainerAlias(name={name}, container={container})".format(
            name=repr(self.name), container=repr(self.container)
        )
...
class PublicationQuerySet(models.QuerySet):
    ...
    def aggregated_list(self, identifier=None, **kwargs):
        ...
        if identifier in ["sponsors", "platforms", "container"]:
            return (
                self.primary(prefetch=True, **kwargs)
                .annotate(name=F(identifier + "__name"))
                .values("name")
                .order_by("name")
                .annotate(
                    published_count=Count("name"),
                ...
                .values("name", "published_count", "code_availability_count")
    ...
    def get_top_records(self, attribute=None, number=10):
        ...
        if attribute is None:
            return list(
                Publication.objects.filter(is_primary=True, status="REVIEWED")[:number]
            )
        try:
            records = (
                self.primary(status="REVIEWED")
                .values(attribute)
                .order_by(attribute)
    ...
    def has_no_archive_urls(self):
        return self.annotate(
            code_archive_urls_count=models.Count("code_archive_urls")
        ).filter(code_archive_urls_count=0)

    def has_unavailable_archive_urls(self):
        return self.annotate(
            unavailable_archive_urls=models.Count(
                "code_archive_urls",
    ...
    def no_code_available(self, **kwargs):
        return (
            ...
            .filter(has_available_code=False)
        )

    def by_code_archive_url_status(
        self, status=None, number_of_authors=10, contact_email=None, **kwargs
    ):
        """
        ...
        qs = qs.exclude(
            pk__in=list(
                AuthorCorrespondenceLog.objects.values_list("publication", flat=True)
            )
        )
        return qs

    def eligible_authors(self, order_by="-author_count", **kwargs):
        already_contacted_authors = AuthorCorrespondenceLog.objects.values_list(
            "publication__contact_email"
        ).distinct()
        return (
            self.reviewed(**kwargs)
            .exclude(contact_email="")
            .exclude(contact_email__in=already_contacted_authors)
            .values_list("contact_email")
            .distinct()
            .annotate(author_count=Count("contact_email"))
            .order_by(order_by)
        )
        ...
        return (
            self.annotate(
                available_code_archive_urls_count=models.Count(
                    "code_archive_urls",
                    filter=models.Q(
                        code_archive_urls__status__in=("available", "restricted")
                    ),
                )
            )
            .annotate(
            ...
            .annotate(
                has_available_code=models.Case(
                    models.When(
                        models.Q(available_code_archive_urls_count__gt=0)
...
class Publication(AbstractLogModel):
    Status = Choices(
        ...
        (
            "INVALID",
            _(
                "Not applicable: Publication does not refer to a specific computational model"
            ),
        ),
        (
            "REVIEWED",
            _("Reviewed: Publication metadata reviewed and verified by CoMSES"),
        ),
    )
    ...
    zotero_date_added = models.DateTimeField(
        help_text=_("date added field from zotero"), null=True, blank=True
    )
    zotero_date_modified = models.DateTimeField(
        help_text=_("date modified field from zotero"), null=True, blank=True
    )
    creators = models.ManyToManyField(
        Author, related_name="publications", through="PublicationAuthors"
    )
    ...
    platforms = models.ManyToManyField(
        Platform,
        blank=True,
        through="PublicationPlatforms",
        related_name="publications",
    )
    sponsors = models.ManyToManyField(
        Sponsor, blank=True, through="PublicationSponsors", related_name="publications"
    )
    model_documentation = models.ManyToManyField(
        ModelDocumentation,
        through="PublicationModelDocumentations",
        blank=True,
        related_name="publications",
    )
    tags = models.ManyToManyField(
        Tag, through="PublicationTags", blank=True, related_name="publications"
    )
    added_by = models.ForeignKey(
        User, related_name="citation_added_publication_set", on_delete=models.PROTECT
    )
    ...
    container = models.ForeignKey(
        Container, related_name="publications", on_delete=models.PROTECT
    )
    ...
    citations = models.ManyToManyField(
        ...
        through="PublicationCitations",
        through_fields=("publication", "citation"),
    )
    ...
    def duplicates(self, query=None, container=None, **kwargs):
        ...
        if container is not None:
            if container.id is not None:
                criteria |= (
                    Q(container_id=container.id)
                    & Q(title__iexact=self.title)
                    & ~Q(title="")
                )
            elif container.issn:
                criteria |= (
                    Q(container__issn=container.issn)
                    & Q(title__iexact=self.title)
                    & ~Q(title="")
                )

        if query is None:
            query = Publication.objects.filter(criteria).exclude(id=self.id)
    ...
    @property
    def is_archived(self):
        return self.code_archive_urls.exclude(
            status=CodeArchiveUrl.STATUS.unavailable
        ).exists()
    ...
    def slug(self):
        ...
        apa_authors = "-".join(
            ["{0} {1}".format(c.given_name, c.family_name) for c in self.creators.all()]
        )
        slug_text = self.slugify_max(
            "-".join([x for x in [apa_authors, year_str, self.title] if x]), 100
        )
        return slug_text
    ...
    def __str__(self):
        return "id: {id} {title} {year}. {container}".format(
            id=self.id,
            title=self.title,
...
class CodeArchiveUrlPattern(models.Model):
    ...
    category = models.ForeignKey(
        CodeArchiveUrlCategory, on_delete=models.PROTECT, related_name="patterns"
    )
...
class CodeArchiveUrl(AbstractLogModel):
    STATUS = Choices(
        (
            "available",
            _(
                "Available: Codebase is currently openly accessible at the specified URL"
            ),
        ),
        (
            "restricted",
            _("Restricted: Specified URL is locked behind authentication or paywall"),
        ),
        ("unavailable", _("Unavailable: No URL available or URL does not resolve")),
    )

    publication = models.ForeignKey(
        Publication, on_delete=models.PROTECT, related_name="code_archive_urls"
    )
    ...
    is_active = models.BooleanField(
        default=True,
        help_text=_(
            "True if this Archive URL is not defunct, i.e., has been replaced."
        ),
    )
    notes = models.TextField(
        blank=True, help_text=_("Remarks on this URL from the author or curator")
    )
    ...
    system_overridable_category = models.BooleanField(
        default=True,
        help_text=_(
            "Signifies that this URL's category can be overridden (i.e., not user entered)."
        ),
    )
    ...
    def check_status(self, patterns, fallback_category):
        url = self.url
        category = CodeArchiveUrl.categorize_url(
            url, patterns, fallback_category=fallback_category
        )
        try:
    ...
    def category_name(self):
        return (
            f"{self.category.category} / {self.category.subcategory}"
...
class URLStatusLog(models.Model):
    ...
    date_created = models.DateTimeField(
        auto_now_add=True, help_text=_("Date this url was last verified")
    )
    ...
    headers = models.TextField(
        blank=True, help_text=_("contains information about the url header")
    )
    status_code = models.PositiveIntegerField(default=0)
    status_reason = models.TextField(
        blank=True, help_text=_("contains reason for the url success/failure")
    )
    system_generated = models.BooleanField(default=True)

    def get_message(self):
        return "Pub: {pub} {url} {code} {reason}".format(
...
class AuditCommand(models.Model):
    ...
    message = models.TextField(
        blank=True, help_text=_("A human readable representation of the change made")
    )
    ...
    def __str__(self):
        return (
            f"action={self.action} date_added={self.date_added} creator={self.creator}"
        )
    ...
    def get_contributor_data(self, publication):
        audit_logs = (
            AuditLog.objects.filter(
                Q(audit_command__action="MANUAL")
                & (
                    Q(table=publication._meta.model_name, row_id=publication.id)
                    | Q(pub_id=publication.id)
                )
            )
            .annotate(creator=F("audit_command__creator__username"))
            .values("creator")
            .order_by("creator")
        )
        ...
        unique_logs = (
            audit_logs.annotate(
                contribution=(
                    Cast((Count("creator")) * 100 / len(audit_logs), IntegerField())
                ),
                date_added=(Max("audit_command__date_added")),
            )
            .values("creator", "contribution", "date_added")
            .order_by("-date_added")
        )
...
class AuditLog(models.Model):
    ...
    Action = Choices(
        ("UPDATE", _("Update")), ("INSERT", _("Insert")), ("DELETE", _("Delete"))
    )
    action = models.CharField(max_length=64, choices=Action)
    ...
    payload = JSONField(
        ...
        null=True,
        help_text=_(
            "A JSON dictionary containing modified fields, if any, for the given publication"
        ),
    )
    pub_id = models.ForeignKey(
        Publication,
    ...
    audit_command = models.ForeignKey(
        AuditCommand, related_name="auditlogs", on_delete=models.CASCADE
    )
    message = models.CharField(max_length=2000, blank=True)
    objects = AuditLogQuerySet.as_manager()

    def __str__(self):
        return "{} performed {} on {}".format(
            self.action,
            self.message,
            self.payload,
        )
...
class Raw(AbstractLogModel):
    ...
    publication = models.ForeignKey(
        Publication, related_name="raw", on_delete=models.PROTECT
    )
    container = models.ForeignKey(
        Container, related_name="raw", on_delete=models.PROTECT
    )
...
class PublicationAuthors(AbstractLogModel):
    ...
    author = models.ForeignKey(
        Author, related_name="publication_authors", on_delete=models.CASCADE
    )
    role = models.CharField(choices=RoleChoices, max_length=64)
    corresponding_author = models.BooleanField(
        default=False, help_text=_("True if this author can be contacted.")
    )
...
class PublicationCitations(AbstractLogModel):
    publication = models.ForeignKey(
        Publication, related_name="publication_citations", on_delete=models.CASCADE
    )
    citation = models.ForeignKey(
        Publication,
        related_name="publication_citations_referenced_by",
...
class PublicationModelDocumentations(AbstractLogModel):
    publication = models.ForeignKey(
        Publication,
        related_name="publication_modeldocumentations",
        on_delete=models.CASCADE,
    )
    model_documentation = models.ForeignKey(
        ModelDocumentation,
        related_name="publication_modeldocumentations",
        on_delete=models.CASCADE,
    )
...
class PublicationPlatforms(AbstractLogModel):
    publication = models.ForeignKey(
        Publication, related_name="publication_platforms", on_delete=models.CASCADE
    )
    platform = models.ForeignKey(
        Platform, related_name="publication_platforms", on_delete=models.CASCADE
    )
...
class PublicationSponsors(AbstractLogModel):
    publication = models.ForeignKey(
        Publication, related_name="publication_sponsors", on_delete=models.CASCADE
    )
    sponsor = models.ForeignKey(
        Sponsor, related_name="publication_sponsors", on_delete=models.CASCADE
    )
...
class PublicationTags(AbstractLogModel):
    publication = models.ForeignKey(
        Publication, related_name="publication_tags", on_delete=models.CASCADE
    )
    tag = models.ForeignKey(
        Tag, related_name="publication_tags", on_delete=models.CASCADE
    )
...
class RawAuthors(AbstractLogModel):
    author = models.ForeignKey(
        Author, related_name="raw_authors", on_delete=models.CASCADE
    )
...
class Submitter(models.Model):
    ...
    def __repr__(self):
        return (
            f"<Submitter: user={self.user}>"
            if self.user
            else f"<Submitter: email={self.email}>"
        )


class SuggestedPublication(models.Model):
    doi = fields.NonEmptyTextField(
        max_length=255, unique=True, verbose_name=_("DOI"), blank=True
    )
...
class SuggestedMerge(AbstractLogModel):
    ...
    creator = models.ForeignKey(
        Submitter, related_name="suggested_merge_set", on_delete=models.PROTECT
    )
    ...
    def annotate_names(cls, instances: List["SuggestedMerge"]):
        ...
        for model, ids in model_lookups.items():
            ...
            matching_instances = [
                instances[ind] for ind in model_class_to_indices[model]
            ]
            for instance_match in matching_instances:
                instance_match.duplicate_instances = [
                    pk_to_related_instances[pk]
                    for pk in instance_match.duplicates
    ...
    def duplicate_text(self):
        ...
        return template.render(
            context={"instances": instances, "missing_pks": missing_pks}
        )
    ...
    def get_message(self):
        ...
        return "{} with duplicates {}".format(
            self.content_type, [instance.get_message() for instance in instances]
        )

    @staticmethod
    def _move_author_aliases(kept_pk, discarded_pks, audit_command):
        kept_author_aliases = set(
            AuthorAlias.objects.filter(author_id=kept_pk).values_list(
        ...
        for author_alias in author_aliases:
            if (
                author_alias.family_name,
                author_alias.given_name,
            ) in kept_author_aliases:
    ...
    def _move_raw_authors(kept_pk, discarded_pks, audit_command):
        kept_raw_ids = set(
            ra.raw_id for ra in RawAuthors.objects.filter(author_id=kept_pk)
        )
    ...
    def _move_publication_authors(kept_pk, discarded_pks, audit_command):
        kept_publication_ids = set(
            pa.publication_id
            for pa in PublicationAuthors.objects.filter(author_id=kept_pk)
        )
    ...
    def merge_authors(cls, pks, content, audit_command):
        ...
        cls._move_publication_authors(
            kept_author_pk, discarded_author_pks, audit_command
        )
    ...
    def merge_containers(pks, content, audit_command):
        ...
        Publication.objects.filter(container__in=discarded_container_pks).log_update(
            audit_command, container_id=kept_container_pk
        )
        Container.objects.filter(pk__in=discarded_container_pks).log_delete(
            audit_command
        )
        Container.objects.get(pk=kept_container_pk).log_update(audit_command, **content)

    @staticmethod
    def _move_publication_platforms(
        kept_platform_pk, discarded_platform_pks, audit_command
    ):
        kept_publication_pks = set(
            PublicationPlatforms.objects.filter(
                platform_id=kept_platform_pk
            ).values_list("publication_id", flat=True)
        )
        publication_platforms = PublicationPlatforms.objects.filter(
            platform_id__in=discarded_platform_pks
        )
        for publication_platform in publication_platforms:
            if publication_platform.publication_id in kept_publication_pks:
                ...
                publication_platform.log_update(
                    audit_command, platform_id=kept_platform_pk
                )
    ...
    def merge_platforms(cls, pks, content, audit_command):
        ...
        cls._move_publication_platforms(
            kept_platform_pk=kept_platform_pk,
    ...
    def _move_publication_sponsors(kept_pk, discarded_pks, audit_command):
        kept_publication_pks = set(
            PublicationSponsors.objects.filter(sponsor_id=kept_pk).values_list(
                "publication_id", flat=True
            )
        )
        publication_sponsors = PublicationSponsors.objects.filter(
            sponsor_id__in=discarded_pks
        )
    ...
    def merge_sponsors(cls, pks, content, audit_command, message=""):
        ...
        logger.info(
            "Moving discarded %s to %s sponsor", discarded_sponsor_pks, kept_sponsor_pk
        )
        cls._move_publication_sponsors(
            kept_pk=kept_sponsor_pk,
    ...
    def _move_publication_tags(kept_pk, discarded_pks, audit_command):
        kept_publication_pks = set(
            PublicationTags.objects.filter(tag_id=kept_pk).values_list(
                "publication_id", flat=True
            )
        )
