...
class PublicationAuditCommand:
    ...
    def many_from_queryset(cls, auditlogs):
        ...
        in_bulk_audit_commands = AuditCommand.objects.select_related("creator").in_bulk(
            audit_command_ids
        )
        ...
        for id, audit_command in in_bulk_audit_commands.items():
            ...
            publication_audit_command = cls(
                id=id,
                creator=audit_command.creator.username,
...
def publication_audit_command_serializer(auditlogs):
    ...
    for id, audit_command in in_bulk_audit_commands.items():
        auditlogs = partioned_auditlogs[id]
        publication_audit_command = dict(
            id=id,
            creator=audit_command.creator.username,
            action=audit_command.action,
            auditlogs=auditlogs,
...
class CodeArchiveUrlSerializer(serializers.ModelSerializer):
    id = serializers.ModelField(
        model_field=CodeArchiveUrl()._meta.get_field("id"), allow_null=True
    )
    creator = serializers.PrimaryKeyRelatedField(
        allow_null=True, queryset=User.objects.all()
    )
    publication = serializers.PrimaryKeyRelatedField(
        allow_null=True, queryset=Publication.api.primary(), required=False
    )
...
class PublicationSerializer(serializers.ModelSerializer):
    ...
    def get_activity_logs(self, obj):
        audit_logs = AuditLog.objects.filter(
            Q(table=obj._meta.model_name, row_id=obj.id)
    ...
    def get_code_archive_status_options(self, obj):
        return [
            ...
            for choice in CodeArchiveUrl.STATUS
        ]
    ...
    def save_model_documentation(audit_command, publication, raw_model_documentations):
        names = [
            model_documentation_raw["name"]
            for model_documentation_raw in raw_model_documentations
        ]
        for name in names:
            model_documentation = ModelDocumentation.objects.get(name=name)
            PublicationModelDocumentations.objects.log_get_or_create(
                audit_command=audit_command,
                publication=publication,
                publication_id=publication.id,
                model_documentation_id=model_documentation.id,
            )
        PublicationModelDocumentations.objects.exclude(
            model_documentation__in=ModelDocumentation.objects.filter(name__in=names)
        ).filter(publication=publication).log_delete(audit_command=audit_command)

    @staticmethod
    def save_platform(audit_command, publication, raw_platforms):
        names = [raw_platform["name"] for raw_platform in raw_platforms]
        for name in names:
            platform, created = Platform.objects.log_get_or_create(
                audit_command=audit_command, publication=publication, name=name
            )
            PublicationPlatforms.objects.log_get_or_create(
                audit_command=audit_command,
                publication=publication,
                publication_id=publication.id,
                platform_id=platform.id,
            )
        PublicationPlatforms.objects.exclude(
            platform__in=Platform.objects.filter(name__in=names)
        ).filter(publication=publication).log_delete(audit_command=audit_command)
    ...
    def save_sponsor(audit_command, publication, raw_sponsors):
        ...
        for name in names:
            platform, created = Sponsor.objects.log_get_or_create(
                audit_command=audit_command, publication=publication, name=name
            )
            PublicationSponsors.objects.log_get_or_create(
                audit_command=audit_command,
                publication=publication,
                publication_id=publication.id,
                sponsor_id=platform.id,
            )
        PublicationSponsors.objects.exclude(
            sponsor__in=Sponsor.objects.filter(name__in=names)
        ).filter(publication=publication).log_delete(audit_command=audit_command)
    ...
    def save_code_archive_url(audit_command, publication, raw_code_archive_urls):
        ...
        for raw_code_archive_url in raw_code_archive_urls:
            ...
            if pk is not None:
                ...
                code_archive_url = CodeArchiveUrl.objects.log_create(
                    audit_command=audit_command,
    ...
    @staticmethod
    def pop_related_validated_data(validated_data):
        return {
            "model_documentation": validated_data.pop("model_documentation"),
            "platforms": validated_data.pop("platforms"),
            "sponsors": validated_data.pop("sponsors"),
            "code_archive_urls": validated_data.pop("code_archive_urls"),
        }
    ...
    def create(self, audit_command, validated_data):
        ...
        related_data = self.pop_related_validated_data(validated_data)

        instance = ModelClass.objects.log_create(
            audit_command=audit_command, **validated_data
        )
    ...
    def update(self, audit_command, instance, validated_data):
        ...
        raw_model_documentations = related_data["model_documentation"]
        self.save_model_documentation(
            audit_command=audit_command,
            publication=instance,
            raw_model_documentations=raw_model_documentations,
        )

        raw_platforms = related_data["platforms"]
        self.save_platform(
            audit_command=audit_command,
            publication=instance,
        ...
        raw_sponsors = related_data["sponsors"]
        self.save_sponsor(
            audit_command=audit_command, publication=instance, raw_sponsors=raw_sponsors
        )

        raw_code_archive_urls = related_data["code_archive_urls"]
        self.save_code_archive_url(
            audit_command=audit_command,
            publication=instance,
        ...
        for field_name, updated_data_value in validated_data.items():
            try:
                ...
                raise ValidationError(
                    "'{0}' not a field of publication with id={1} and title={2}".format(
    ...
    def save(self, user=None, **kwargs):
        # Keep explicit audit command ownership while avoiding direct reliance
        # on private DRF internals.
        user = user or kwargs.pop("user", None)
        if user is None:
            raise TypeError(
                "'user' is required when saving PublicationSerializer. "
    ...
    def modified_data_text(self):
        ...
        mdl = [
            ...
            for key, pair in self.modified_data.items()
        ]
        return " | ".join(mdl)
    ...
    class Meta:
        ...
        fields = (
            ...
            "doi",
            "code_archive_urls",
            "code_archive_status_options",
            "code_archive_category_options",
        )
...
class ContactFormSerializer(serializers.Serializer):
    ...
    def validate(self, data):
        ...
        if security_hash == new_security_hash:
            return data
        logger.warning("timestamp was altered, flagging as invalid")
...
class SuggestMergeSerializer(serializers.Serializer):
    ...
    def validate(self, data):
        ...
        if data["model_name"] == "author":
            ...
            serializer.is_valid(raise_exception=True)
        data["new_content"] = serializer.validated_data
        return data
